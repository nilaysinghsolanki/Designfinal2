...
class _AddEventScreenState extends State<AddEventScreen> {
  ...
  Future getImage() async {
    ...
    try {
      ...
      _imageDataList = [await _load(File(pickedFile.path))];
    } catch (e) {
      print('c');
      print(e.code);
    }
  ...
  Future<Uint8List> _load(File image) async {
    final assetData = await rootBundle.load(image.path);
          ...
          ? Container(
              ...
              child: Crop(
                ...
                onCropped: (croppedImage) async {
                  setState(() {
                    _image = File.fromRawPath(croppedImage);

                    imageCroped = true;
                  });
                  _imageDataList = [await _load(_image)];
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: ListTile(
                        ...
                        title: Text('Duration?'),
                        trailing:
                            Text('${data.getHours()}h ${data.getMinutes()}min'),
                        onTap: () {
                          showDialog(
                              context: context,
                              builder: (context) {
